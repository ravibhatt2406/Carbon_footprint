...
export default function StatCard({ title, value, unit, icon: IconComponent, trend, trendValue, colorClass }) {
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex flex-col justify-between hover:shadow-md transition-shadow duration-300">
      ...
      {trendValue !== undefined && trendValue !== null && (
        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center text-xs">
          ...
          <span className="text-slate-400 font-medium ml-2">from last record</span>
        </div>
      )}
    </div>
  );
