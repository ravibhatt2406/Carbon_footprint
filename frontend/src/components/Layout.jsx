...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      {/* SIDEBAR FOR DESKTOP */}
      <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 bg-forest-950 text-white border-r border-forest-900 shadow-xl z-30">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <IconComponent className={`h-5 w-5 ${active ? 'text-white' : 'text-forest-400 group-hover:text-emerald-400 transition-colors'}`} />
                <span>{item.name}</span>
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t border-forest-900 bg-forest-950/40">
          <div className="flex items-center justify-between p-3 rounded-2xl bg-forest-900/40 border border-forest-900/30">
            <div className="flex items-center space-x-2.5 overflow-hidden">
              <div className="h-9 w-9 rounded-xl bg-emerald-500 flex items-center justify-center font-bold text-white uppercase text-sm shrink-0">
      ...
      {/* MOBILE HEADER */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-forest-950 text-white flex items-center justify-between px-6 shadow-md z-40">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-16 bg-forest-950/95 backdrop-blur-sm z-40 flex flex-col pt-4 px-6 pb-6 text-white border-t border-forest-900">
          ...
          <div className="pt-6 border-t border-forest-900 mt-6">
            <div className="flex items-center justify-between bg-forest-900/40 p-4 rounded-xl">
              <div className="flex items-center space-x-3">
                <div className="h-9 w-9 rounded-lg bg-emerald-500 flex items-center justify-center font-bold text-white uppercase text-sm">
                  {user?.displayName?.[0] || 'U'}
                </div>
                <div>
                  <p className="text-sm font-semibold truncate">{user?.displayName}</p>
                  <p className="text-xs text-emerald-400 font-bold">🌱 {user?.points || 0} Points</p>
                </div>
              </div>
              <button
                onClick={handleLogout}
